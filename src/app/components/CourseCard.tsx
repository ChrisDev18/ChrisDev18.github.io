...
export default function CourseCard({title, description, Icon, year, language}: CourseCardProps) {
  ...
  return (
      <motion.div
          ...
          whileHover="hover"
          whileFocus="hover"
          className="text-white group relative border hover:border-white/50 hover:shadow-[0px_2px_24px_0px_#8F5FFF88] border-white/20 p-5 pt-8 w-80 h-96 flex flex-col items-center gap-4 bg-black hover:bg-[#0C0814] transition-colors duration-75"
      >

        {year && language &&
          <div className="absolute top-0 left-0 flex flex-col items-start">
            { year &&
              <p className={`border-white/20 group-hover:border-white/50 border-b border-r py-2 px-3 lining-nums font-sans`}>{year}</p>
            }
            { language &&
              <p className={`border-white/20 group-hover:border-white/50 border-b border-r py-2 px-3 font-sans`}>{language}</p>
            }
          </div>
        }

        <div className="flex flex-col items-center gap-2 group-hover:pointer-events-none">
          <motion.span variants={iconVariants}>
            <Icon className="relative rotate-4" size={64} strokeWidth={1} />
          </motion.span>
          <motion.svg variants={pedestalVariants} width="146" height="22" viewBox="0 0 146 22" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <span className="flex h-[64px] items-center">
          <h3 className={`text-2xl font-semibold text-center font-sans`}>{title}</h3>
        </span>
        <p className={`font-medium text-center font-sans`}>{description}</p>

        {/*<Hyperlink href={"#"} internal chevron={false} className={`font-bold text-center font-sans`}>View <br/> coursework</Hyperlink>*/}
